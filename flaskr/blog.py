...
def create():
    ...
    if request.method == 'POST':
        ...
        cur.execute('SELECT title FROM post WHERE title = %s', title)
        ...
        if error is not None:
            ...
            db = get_db()
            db.cursor().execute(
                'INSERT INTO post (title, body, author_id) VALUES ("{0}", "{1}", "{2}")'
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        cur.execute('SELECT id FROM post WHERE title = %s', title)
        newId = cur.fetchone()
        ...
        if error is not None:
            ...
            cur.execute(
                'UPDATE post SET title = "{0}", body = "{1}" WHERE id = {2}'
